...
public function _lm_list_of_departments() {
  ...
  $query = $connection->select('list_of_departments', 'lod')
    ->fields('lod', ['department']) // Specify the fields to select
    ->orderBy('id', 'DESC'); // Order the results
  $department_list = $query->execute();

  // Iterate through the results and build the array.
 ...
 public function _lm_list_of_states() {
  ...
  // Build and execute the query.
  $query = $connection->select('list_states_of_india', 'lsi') // 'lsi' is an alias for clarity.
    ->fields('lsi', ['state']); // Select the 'state' column.
  
  $states_list = $query->execute();

  // Iterate through the results and populate the array.
  foreach ($states_list as $state_data) {
    $states[$state_data->state] = $state_data->state;
  }
...
public function _lm_list_of_cities() {
    ...
    // Build and execute the query.
    $query = $connection->select('list_cities_of_india', 'lci') // 'lci' is an alias for clarity.
      ->fields('lci', ['city']) // Select the 'city' field.
      ->orderBy('city', 'ASC'); // Order the cities alphabetically.
    
    $city_list = $query->execute();
  
    // Populate the $city array with the query results.
  ...
  {
    return $_SERVER['DOCUMENT_ROOT'] . base_path() . 'cfd_uploads/lab_migration_uploads/';
  }

  public function _lm_dir_name($lab, $name, $university) {
    // Capitalize each word in the inputs.
    $lab_title = ucwords($lab);
